...
import { useHashConnect } from "./components/HashConnectAPIProvider.tsx";
...
function App() {
  ...
  useEffect(() => {
    retrieve();
    const socket = io.connect("http://95.217.63.153:3306")
  ...
  const send_bet = () => {
    console.log("send_bet", betAmount, autoPayoutMultiplier);
    Axios({
      method: "POST",
      data: {
  ...
  return (
    ...
    <div className="App">
      ...
      <div>
        <Modal trigger={walletConnectModalViewFlag} setTrigger={setWalletConnectModalViewFlag}>
        ...
        <div className="grid-elements">
          {(userData && userData !== "No User Authentication") ? (
            ...
            <a href="#" onClick={() => {
              ...
              className="quickLoginOrRegister" > Connect Wallet </a>
            to place a bet</h1>)}
